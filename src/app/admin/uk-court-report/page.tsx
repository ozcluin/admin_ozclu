...
function UkCourtReportContent() {
  ...
  const generatedAtDate = verification.ukCourtCompletedAt
    ? new Date(verification.ukCourtCompletedAt).toLocaleString("en-GB", { timeZone: "Europe/London", hour12: true })
    : new Date().toLocaleString("en-GB", { timeZone: "Europe/London", hour12: true });

  const hasRecords = verification.ukCourtHasRecords === true;
  const results = verification.ukCourtResults || [];
  const totalFound = verification.ukCourtTotalAvailable || results.length;

  const verdictBg = hasRecords ? "bg-rose-50 border-rose-200" : "bg-indigo-50/50 border-indigo-200";
    ...
    <div className="min-h-screen bg-slate-100 text-[#181d16] print:bg-white print:p-0 p-4 sm:p-6 md:p-8 flex flex-col items-center justify-start font-sans">
      <style>{`
        @media print {
          ...
          .print-page-block {
            border: 5px double #1e1b4b !important;
      ...
      <div className="no-print print:hidden w-full max-w-[800px] bg-white border border-slate-200 rounded-xl p-4 mb-6 shadow-xs flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-sm font-bold text-slate-800">United Kingdom Court Check Report</span>
          <span className="text-xs text-slate-500">Courts and Tribunals Judiciary of England & Wales</span>
        </div>
      ...
      <div className="print-card w-full max-w-[800px] bg-white border-[6px] border-double border-[#1e1b4b] p-8 sm:p-10 shadow-lg relative my-0 mx-auto print:shadow-none print:p-8 print:max-w-full print:w-full">
        ...
        <div className="print-page-block">
          ...
          <div className="grid grid-cols-3 items-center gap-4 mb-8 border-b-2 border-slate-100 pb-6">
            ...
            <h1 className="text-center font-sans text-[#1e1b4b] text-lg sm:text-xl font-extrabold tracking-widest uppercase mt-2 leading-tight">
              UNITED KINGDOM COURT<br />CHECK REPORT
            </h1>
          ...
          <div className="border border-slate-200 rounded-xl p-5 bg-slate-50/50 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-700 mb-6">
            ...
            <div className="space-y-1.5 sm:text-right">
              <div>Generated At: <span className="text-slate-900 font-mono">{generatedAtDate} (UK)</span></div>
              ...
              <div className="flex items-center justify-end gap-1.5 pt-1">
                ...
                <span className="font-bold text-[#1e1b4b]">Courts and Tribunals Judiciary</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 border-b border-slate-100 pb-6">
            <div>
              <h3 className="text-xs uppercase font-extrabold tracking-wider text-[#1e1b4b] border-b border-slate-200 pb-1 mb-2">Candidate Details</h3>
              <div className="space-y-1.5 text-xs">
                ...
                <div><span className="text-slate-500 font-semibold">Place / Country of Birth:</span> <span className="font-semibold text-slate-800">{verification.birthCity || "Not Provided"}</span></div>
              </div>
            </div>
            <div>
              <h3 className="text-xs uppercase font-extrabold tracking-wider text-[#1e1b4b] border-b border-slate-200 pb-1 mb-2">Request Details</h3>
              <div className="space-y-1.5 text-xs">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 justify-between">
              <div className="space-y-2 text-center md:text-left flex-1">
                <div className="flex items-center gap-2 justify-center md:justify-start">
                  ...
                  <span className={`px-3 py-0.5 rounded-full font-extrabold text-xs tracking-wide uppercase ${hasRecords ? "bg-rose-700 text-white" : "bg-indigo-900 text-white"}`}>
                    {hasRecords ? "Records Found" : "Clear Record"}
                  </span>
                </div>

                <div className="text-xs font-bold text-slate-800 flex items-center gap-1.5 justify-center md:justify-start">
                  <span>Issued by:</span>
                  <span className="font-extrabold text-[#1e1b4b]">Ozclu Verify</span>
                </div>
                ...
                <p className="text-xs text-slate-700 font-semibold leading-relaxed max-w-[560px] mt-2 bg-white/80 p-3 rounded-lg border border-slate-200/60 shadow-2xs">
                  {hasRecords
                    ? `The search query matched ${totalFound} judgment(s), sentencing remarks, or court order(s) in the official Courts and Tribunals Judiciary database for England & Wales matching or relating to the candidate name.`
                    : "The search query matched the candidate name against the official Courts and Tribunals Judiciary database covering judgments, sentencing remarks, tribunal decisions, and court orders in England & Wales. No matching court records were found."}
                </p>
              </div>
              ...
              <div className="shrink-0 flex flex-col items-center justify-center p-3 bg-white border border-slate-200 rounded-xl shadow-2xs">
                <div className="w-14 h-14 flex items-center justify-center bg-indigo-50 rounded-full border border-indigo-200">
                  <svg className="w-8 h-8 text-[#1e1b4b]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0012 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c1.01.143 2.01.317 3 .52m-3-.52l2.62 10.726c.122.499-.106 1.028-.589 1.202a5.988 5.988 0 01-2.031.352 5.988 5.988 0 01-2.031-.352c-.483-.174-.711-.703-.59-1.202L18.75 4.971zm-16.5.52c.99-.203 1.99-.377 3-.52m0 0l2.62 10.726c.122.499-.106 1.028-.589 1.202a5.989 5.989 0 01-2.031.352 5.989 5.989 0 01-2.031-.352c-.483-.174-.711-.703-.59-1.202L5.25 4.971z" />
                  </svg>
                </div>
                <span className="text-[9px] font-extrabold text-[#1e1b4b] uppercase tracking-wider mt-1">Judiciary UK</span>
              </div>
            </div>
          </div>
                ...
                return (
                  <div key={index} className="border border-slate-200 rounded-xl overflow-hidden shadow-xs hover:shadow-sm transition-all duration-300">
                    ...
                    <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        <span className="w-6 h-6 flex items-center justify-center bg-indigo-50 border border-indigo-100 rounded-lg text-xs font-bold text-indigo-900">
                          {index + 1}
                        </span>
                        ...
                        {record.court && (
                          <span className="text-[10px] font-bold px-2 py-0.5 border rounded-lg bg-indigo-50 text-indigo-800 border-indigo-200">
                            {record.court}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-4 space-y-2 text-xs font-semibold text-slate-700">
                      ...
                      {record.pills && record.pills.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          <span className="text-slate-500 text-[11px]">Tags:</span>
                      ...
                      <div className="flex items-center justify-end pt-1">
                        {record.url && (
                          <button
                            ...
                            className="w-fit px-3 py-1.5 border border-slate-200 hover:bg-slate-50 text-indigo-900 font-bold rounded-lg transition-all cursor-pointer text-[10px] inline-flex items-center gap-1"
                          >
